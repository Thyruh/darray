...
int main(void) {
   ...
   printf("%d\n", *i32_safe_at(Darray_i32_at(&v_i32s, 0), 2));

   printf("-------------------------------------- Full darray of integers ----------------------------------\n");

   printf("elements: ");
   for (u32 i = 0; i < i32s.size-1; i++) {
      printf("%d, ", *i32_safe_at(&i32s, i));
   }
   printf("%d.\n", *i32_safe_at(&i32s, i32s.size-1));
   printf("length = %d\n", i32s.size);

   i32_pop(&i32s, 10);  // delete the last 10 elements of the darray
